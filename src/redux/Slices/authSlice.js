...
const API_BASE =
  "https://qpcfbzrk62.execute-api.eu-north-1.amazonaws.com/api/v1/";
...
export const sendOtp = createAsyncThunk(
  ...
  async (mobileNumber, { rejectWithValue }) => {
    try {
      const response = await axios.post(`${API_BASE}auth/sendLoginOtp`, {
        mobileNumber,
      });
...
export const verifyOtp = createAsyncThunk(
  "auth/verifyLoginOtp",
  async ({ id, otp }, { rejectWithValue, dispatch }) => {
    try {
      const response = await axios.post(`${API_BASE}auth/verifyLoginOtp`, {
        id,
        otp,
      });
...
export const registerUserSendOtp = createAsyncThunk(
  "auth/userRegister",
...
export const registerUserVerifyOtp = createAsyncThunk(
  ...
  async ({ id, otp }, { rejectWithValue, dispatch }) => {
    try {
      const response = await axios.post(`${API_BASE}auth/otpVerification`, {
        id,
        otp,
      });
      const token = response.data.token;
...
export const { logout, resetLoginState, resetLoading, clearError } =
  authSlice.actions;
