...
export default function BookRun() {
  ...
  const toggleFav = (e, evt) => {
    ...
    if (!evt.favourite) likeEvent.mutate({ eventId: id, userId, type: evt.type }, { onSuccess: () => queryClient.invalidateQueries(["EventList", userId]) });
    else unlikeEvent.mutate({ favouriteEventId: evt.favourite_event_id }, { onSuccess: () => queryClient.invalidateQueries(["EventList", userId]) });
  };
            ...
            >
              ...
              <div className="hs-card-body">
                ...
                <div className="hs-card-meta">
                  ...
                  <span>
                    {new Date(evt.start_date).toLocaleDateString("en-GB", { day: "2-digit", month: "short" })} | {fmt(evt.start_time)}
                  </span>
                </div>
