...
export default function BookVenues() {
  ...
  useEffect(() => { if (data?.status === 200) setVenueList(data.result); }, [data]);
    ...
    <section className="hs-section">
      <Container>
        ...
        <div className="hs-grid" ref={gridRef}>
          {venueList.slice(0, 4).map((venue, idx) => (
            <Link
              to={`/venue/${venue.id}`}
              key={venue.id}
            ...
            >
              ...
              <div className="hs-card-body">
                <p className="hs-card-name">{venue.venue_name}</p>
                ...
                {venue.sports?.length > 0 && (
                  <div className="hs-sports">
                    ...
                    {venue.sports.length > 5 && (
                      <span style={{ fontSize: 10, color: "#858585", alignSelf: "center" }}>+{venue.sports.length - 5}</span>
                    )}
                  </div>
                )}
