import react from "react";
import "../stylesheets/PrivacyAndPolicy.css"

function PrivacyAndPolicy() {

    return (
        <div className="privacy-container">
            <h1>Privacy & Policy</h1>
            {/* <p>Last updated: July 2, 2025</p> */}

            <section>
                <p>In accordance with this privacy policy ("Policy"), Danta Sports Pvt. Ltd. and its colleagues, partners, successors, and authorized assigns ("Danta Sports," "we," "us," and "our") are dedicated to preserving your privacy. In this Policy, "you" refers to the users of the Danta Sports Platform (as defined below), regardless of whether you utilize the Platform's services or complete a transaction there ("Users"). The Policy outlines: (a) the kinds of data we might gather from you when you use our Platform to access or utilize our services (collectively, our "Services"), and (b) how we gather, use, preserve, safeguard, and disclose that data.</p>
                <p>
                    The term "Platform" refers to both the Website and the Danta Sports App, either directly or through licenses that we have granted. You accept the terms and conditions of this Policy by using or accessing this Platform and submitting SPDI (as defined below) or Information (as defined below). Additionally, you indicate your acceptance to this Policy and the Terms of Use (which are incorporated by reference herein) and give your express consent to our using and disclosing your information and/or SPDI in any way as specified in this Policy. Please do not continue, use our platform, or give us any information if you disagree with the terms and conditions of this policy.
                </p>

                <section>
                    <h2>I. Defination</h2>
                    <dl>
                        <div className="privacy-policy">
                            <dt><strong>A.</strong></dt>
                            <dd>
                                The term <em>"Account"</em> denotes the password-protected account that users set up in order to access and engage with the Platform.
                            </dd>
                        </div>
                        <div className="privacy-policy">
                            <dt><strong>B.</strong></dt>
                            <dd>
                                "Non-personal Identification Information" is any non-personal information that is collected from Users as a result of their interaction with the Platform. This information includes, but is not limited to, the type of computer, the browser name, and technical information about the means of connection to the Platform, such as the operating system and the Internet service providers used.
                            </dd></div>

                        <div className="privacy-policy">
                            <dt><strong>C.</strong></dt>
                            <dd>
                                When a user visits the platform, registers, completes a transaction, answers a survey, fills out a form, or does anything else related to other activities, services, features, or resources we make available on the platform, such information is referred to as "Personal Identification Information" because it can be used to identify, contact, or locate the person to whom it relates. In connection with the services offered on the Platform, users may be requested to provide personal information, such as their name, email address, phone number, and residence address. However, users can access our website anonymously. Users' personal identification information will only be collected by Danta Sports if they willingly provide it to us. Except in cases where it could prohibit them from using specific services or activities offered on the Platform, users are always free to decline to provide Personally Identifying Information.
                            </dd></div>

                        <div className="privacy-policy">
                            <dt><strong>D.</strong></dt>
                            <dd>
                                Together, personal and non-personal identification information will be referred to as "Information" for the purposes of this policy.Information pertaining to the following is included in "sensitive personal data or information" (SPDI):

                                <ul>
                                    <li>Passwords.</li>
                                    <li>financial data, including bank account, credit card, debit card, or other payment instrument details.</li>
                                    <li>physical, physiological, and mental health conditions.</li>
                                    <li>sexual orientation.</li>
                                    <li>medical records and history.</li>
                                    <li>biometric data.</li>
                                    <li>any information pertaining to the aforementioned clauses that is given to the body corporate in order for it to provide services.</li>
                                    <li>and any data that the body corporate receives under the aforementioned clauses in order to process, store, or process it under a valid contract or otherwise.</li>
                                </ul>
                            </dd>
                        </div>

                        <div className="privacy-policy">
                            <dt><strong>E.</strong></dt>
                            <dd>
                                A <em>“Third Party”</em> is any individual or organization that is not you or us.
                            </dd>
                        </div>
                    </dl>
                </section>
            </section>



            <section>
                <h2>II. COLLECTION OF PERSONAL INFORMATION BY Danta Sport</h2>
                <dl>
                    <dt><strong> A.	Regarding the utilization of the Platform.</strong></dt>
                    <dd>
                        <ul>
                            <li>Users will need to provide information, including personal contact details, in order to create an account on the platform. Depending on how a user interacts with the platform, several types of information are gathered.</li>
                            <li>We will gather your name, phone number, email address, residential or business address, and other information when you create an account for users. In certain circumstances, we may also ask you to provide demographic data, such as your gender, age, level of education, and other details pertinent to the services you are receiving. Information that you put on the Platform in your offer, profile, requirements, feedback, and other correspondence sites may also be collected by us.</li>
                            <li>In order to personalize our location-enabled services, we may gather and analyze data on the GPS location of your mobile device, including its latitude, longitude, and altitude, as well as the time it was collected. Your device ID and other information we have about you may be linked to your location data. By turning off your device's GPS or any other location-tracking features, you can revoke your consent for this feature.</li>
                            <li>In order to provide specific functions within our app, such assisting you in finding gamers through your contacts and connecting with them, we may also ask for access to your device's contact list. You agree that we may gather and use contact details from your contact list, such as email addresses, phone numbers, and contact names, by giving us access to your contact list.</li>
                            <li>Additionally, we maintain the right to listen in on any dialogue between two users that Danta Sports facilitates via any platform method. This data can be used, among other things, to prevent and monitor misuse, protect user rights, and settle any disputes that may come up.</li>
                        </ul>
                    </dd>

                    <dt><strong>B.	 From third-party site</strong></dt>
                    <dd>
                        <ul>
                            <li>You can also create an account with your current login information from Facebook, Google, Twitter, or other accounts (your "Third-Party Site Account"). You can link your account with Third Party Site Accounts as part of the Platform's functionality by either (i) giving us your login credentials via the Platform or (ii) granting us access to your Third Party Site Account, as allowed by the terms and conditions that apply to each Third Party Site Account. You can connect your account to Third Party Site Accounts as part of the Platform's capabilities by either: (i) giving us the login credentials for your Third Party Site Account via the Platform; or (ii) giving us access to your Third Party Site Account, as allowed by the terms and conditions that apply to each Third Party Site Account. You affirm that you have the right to give us access to your Third Party Account and/or your login credentials (including but not limited to, for the purposes outlined herein), without violating any of the terms and conditions that control how you use the relevant Third Party Account, without requiring us to pay any fees, and without subjecting us to any usage restrictions imposed by the relevant Third Party Site service provider.</li>
                            <li>You will become a member if you choose to register by logging into a Third Party Site Account through our Platform. We will use the Personal Information you have given to the relevant Third Party Site service provider (e.g., your "real" name, email address, profile picture, names of friends, names of groups you belong to, other information you make publicly available through the relevant Third Party Site service provider, and/or other information you give us permission to access by allowing the Third Party Site service provider to provide such information) from your Third Party Site Accounts.You understand that by giving us access to your Third Party Site Accounts, we will access, make available, and store (if applicable, as permitted by the Third Party Site service provider, and as authorized by you) the information in your Third Party Site Accounts so that it is accessible on and through your account on the Platform, subject to the privacy settings you have set in those Third Party Site Accounts and depending on the Third Party Site Accounts you choose.</li>
                        </ul>
                    </dd>

                    <dt><strong>C.	Cookies</strong></dt>
                    <dd>
                        <p>Cookies are little data files that your browser saves. Cookies help us provide you with continuous service by recording different parts of your visit. Among other things, they enable ad networks to show you customized ads that they think will be most relevant to you. When you visit websites owned by third parties or when you use the Platform, we may install cookies in your browser.The information you provide to us is not connected to this anonymous data, which is kept separate. Cookies are used by us to:</p>
                            <ul>
                                <li>Verify your login credentials.</li>
                                <li>activate our security mechanisms;</li>
                                <li>contextualize and display advertisements</li>
                                <li>and enhance and expand the website's functionalities</li>
                            </ul>
                    </dd>
                    <dt><strong>D.</strong></dt>
                    <dd>
                       <p> If you do not want to accept cookies, please feel free to adjust your browser's settings. Please be aware, though, that altering your browser's settings could have an impact on how you use the Platform.
                        The use of cookies by advertising is not covered by this policy.</p>
                    </dd>

                    <dt><strong>E.	SPDI collection. </strong></dt>
                    <dd>
                        <p> Users must provide specific SPDI, such as their credit card or debit card information, when creating an account on the platform. Payment gateways and financial institutions need this information in order to complete transactions. We make care to use reputable payment gateways with industry-accepted credentials and security measures, and we never obtain nor store any SPDI pertaining to financial transactions on our platform. Instead, the payment gateway obtains these SPDIs directly through an interaction with our platform. Depending on how a user interacts with the platform, several types of information are gathered. Danta Sports promises to adhere to reasonable security measures and procedures in order to safeguard the information you supply. Without your prior consent, Danta Sports agrees not to transfer or divulge your SPDI to a third party for any reason.</p>
                    </dd>

                </dl>
            </section>

            <section>
                <h2>III. USE OF PERSONAL INFORMATION BY Danta Sports</h2>
                <dl>
                    <dt><strong>A.</strong></dt>
                    <dd>in order to deliver services on the platform. If we must react to subpoenas, court orders, or other legal processes, your information might be shared in accordance with those subpoenas, court orders, or legal processes—and sometimes without your knowledge. If we are compelled to do so by law, regulation, or any other governmental authority, or if we are working with a government investigation, we will also provide your information—including, but not limited to, your name, city, state, phone number, email address, and activity history on the Platform—to law enforcement agencies or other government officials.
                        <ul>
                            <li>handle and react to user inquiries.</li>
                            <li>Recognize the needs of the users.</li>
                            <li>identify technological issues.</li>
                            <li>Offer customer service to users.</li>
                            <li>fulfill our responsibilities under the terms of the agreement between Danta Sports and the Users, including those related to invoicing and collection.</li>
                            <li>Permit users to engage in interactive features provided by the services.</li>
                            <li>in any other manner specified by Danta Sports when collecting user information.</li>
                            <li>for the purpose of generating revenue and processing data for analytics, affiliate marketing, and advertising.</li>
                            <li>or for any other purpose with the user's permission.</li>
                        </ul>
                    </dd>
                    <dt><strong>B.	Interaction.</strong></dt>
                    <dd>
                        <p> To provide you with information about specific promotions or features the Platform may decide to offer, or about our affiliates, subsidiaries, business partners, advertisers, and sponsors, including company news, updates, and relevant product or service information, we may use phone calls, emails, application notifications, short message services, multimedia message services, or other communication channels. After registering as a user, you might receive such communications, which are meant to show you ads for products and services that you might find interesting. We give comprehensive unsubscribe instructions at the bottom of every email, or the user can get in touch with the relevant person, if they would like to stop getting future emails that aren't about a transaction they completed on the platform.</p> 
                    </dd>
                    <dt><strong>C.</strong></dt>
                    <dd><p> However, we do not sell user-specific information to any third party for the aforementioned reasons, and any information we do share with third parties for these purposes will be on a no-name aggregate basis.</p></dd>
                </dl>
            </section>

            <section>
                <h2>IV.	SHARING OF PERSONAL INFORMATION BY Danta Sports</h2>
                <dl>
                    <dt><strong>A.	disclosure of general information</strong></dt>
                    <dd><p>Users' information is never traded, sold, or rented to outside parties. For the aforementioned reasons, we might provide our trusted affiliates, marketers, and business partners with basic aggregated demographic data on users and visitors. In the event of a merger, divestiture, restructuring, reorganization, dissolution, or other sale or transfer of some or all of Danta Sports's assets, whether as a going concern or as part of bankruptcy, liquidation, or similar proceedings, wherein Information of Users held by Danta Sports are among the transferred assets, Personal Identification Information may, however, be shared if necessary with (a) our subsidiaries or affiliates and (b) a buyer or successor.</p></dd>

                    <dt><strong>B.	Law enforcement and court cases. </strong></dt>
                    <dd><p>If we must react to subpoenas, court orders, or other legal processes, your information might be shared in accordance with those subpoenas, court orders, or legal processes—and sometimes without your knowledge. If we are compelled to do so by law, regulation, or any other governmental authority, or if we are working with a government investigation, we will also provide your information—including, but not limited to, your name, city, state, phone number, email address, and activity history on the Platform—to law enforcement agencies or other government officials.</p></dd>
                    <dt><strong>C.	Regarding the Platform, third-party service providers. </strong></dt>
                    <dd><p>We may share your information with third-party contractors who work for us or on our behalf to deliver the services on the Platform, assist us in communicating with you, or operate the Platform, if required to do so.</p></dd>
                    <dt><strong>D.	With permission.</strong></dt>
                    <dd><p>If you give us your permission, we may share your information and/or SPDI.</p></dd>
                </dl>
            </section>

            <section>
                <h2>V.	SECURITY</h2>
                <p>
                    We work hard to protect your information and SPDI against unauthorized access, alteration, disclosure, and destruction, as well as to preserve its security, integrity, and privacy. We put in place the proper administrative, electronic, and physical protocols to protect and aid in preventing unwanted access and to uphold data security. In addition to using SSL-secured communication channels, encryption, passwords, and physical security measures to protect user information and SPDI, our payment gateway partners also adhere to the PCI standard.
                </p>
                <p>However, since no data protection or transfer technique is 100% safe, we are unable to guarantee perfect security. Additionally, we disclaim all liability for any security lapses and the conduct of any third parties that obtain your information. Many other websites are linked to the Services, and since we have no control over them, we cannot and will not be held accountable for their privacy policies or practices. We shall not be held accountable for any loss, damage, or misuse of your information or SPDI, regardless of what is stated in this policy or elsewhere, if the loss, damage, or misuse is caused by an event outside of our reasonable control.</p>
            </section>

            <section>
                <h2>VI.	UPDATING, DELETING AND AMENDING YOUR INFORMATION</h2>
                <dl>
                   
                   <p>We will make every effort to accurately document the information you give us, including any modifications that may come up. You have the right to examine, update, and modify the information we keep about you. You can also ask that we remove any information that is incorrect, lacking, or unnecessary for legal purposes, or that is being processed in a way that violates any applicable laws.</p>
                   
                   <p>Your ability to review, update, amend, and remove your information may be restricted in the following situations: (a) if doing so would violate the rights or safety of another person or people; (b) if the information you request is related to ongoing or planned legal proceedings between you and us; or (c) if giving us access to it would jeopardize our ability to negotiate with you or conduct an investigation into potential illegal activity. The applicable law, including any statutory retention requirements, also affects your ability to review, update, revise, and delete your information.</p>
                </dl>
            </section>

            <section>
                <h2>VII.	LINK TO THIRD PARTY APPS AND WEBSITES</h2>
                <p>
                    Links to other websites, goods, and services may also be included in or displayed on the Platform and in any correspondence you receive from Third Parties. You understand and accept that, unless otherwise noted, all websites and applications are run by third parties and are not under our control, affiliation, or association. As a result, we make no claims about the terms of use of such websites or applications or the privacy practices or policies of such Third Parties. The privacy policies of these websites will govern the information you provide to these Third Party websites. It is advised that you read the privacy policies of any such websites and applications before using them.
                </p>
            </section>

            <section>
                <h2>VIII.	AMENDMENT TO THE POLICY</h2>
                <p>
                    This policy may be updated at any moment by Danta Sports. Upon doing so, we will change the date at the bottom of this policy and send you an email to let you know about it. To stay updated on any changes to the Policy, we recommend users to visit this page often. You understand and accept that it is your duty to frequently examine this policy to stay informed of any changes. After we make changes to the Policy, your continuing use of our Platform indicates that you agree to those changes.
                </p>
            </section>

            <section>
                <h2>IX.	THIRD PARTY POLICIES</h2>
                <p>
                    We get certain information from you while you use our services, such as your IP address, phone number, email address, and device make and model characteristics. You specifically agree to the sharing and dissemination of the information we gather about you with our agents, business partners, and third-party service providers by using our services. For instance, in order to prevent fraud and provide a PayLater checkout experience, your information could be shared with third-party services like GetSimpl. For further information, please see the third-party privacy policy.
                </p>
            </section>

            <section>
                <h2>X.	CONTACT US</h2>
                <p>
                    You can email us at <a href="contact@dantasports.com">contact@dantasports.com</a> or send a letter to the following address if you have any questions about how we process or use the SPDI you submit or this policy.
                </p>
                <address>
                    <strong>Danta Sports Private Limited</strong><br />
                    No. 13 &amp; 14, Sy No. 151, Neeladri Nagar,<br />
                    Electronics City, Bangalore,<br />
                    Karnataka, India
                </address>

            </section>
        </div>
    )
}

export default PrivacyAndPolicy;